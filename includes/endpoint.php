...
if (!is_dir(IMPORTS_TEMP_DIR.session_id() . '/desktop/chunks'))
    mkdir(IMPORTS_TEMP_DIR.session_id() . '/desktop/chunks', 0755, true);
if (!is_dir(IMPORTS_TEMP_DIR.session_id() . '/desktop/files'))
    mkdir(IMPORTS_TEMP_DIR.session_id() . '/desktop/files', 0755, true);
...
$uploader->allowedExtensions = $_import_allowed_file_extensions; // all files types allowed by default
...
$uploader->chunksFolder = IMPORTS_TEMP_DIR.session_id()."/desktop/chunks";
...
if ($method == "POST") {
    ...
    if (isset($_GET["done"])) {
        $result = $uploader->combineChunks(IMPORTS_TEMP_DIR.session_id()."/desktop/files");
    }
    // Handles upload requests
    else {
        // Call handleUpload() with the name of the folder, relative to PHP's getcwd()
        $result = $uploader->handleUpload(IMPORTS_TEMP_DIR.session_id()."/desktop/files");
...
// for delete file requests
    ...
    $result = $uploader->handleDelete(IMPORTS_TEMP_DIR.session_id()."/desktop/files");
    echo json_encode($result);
}
else {
