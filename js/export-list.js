...
$(function() {
    ...
    $('#exportsTable').bootstrapTable({
        url:                '?action=export_list_get',
        method:             'get', // get, post
...
*/
        ...
        onPreBody: function(data) {
            if ($('.fixed-table-toolbar > .page-list').length)
                $('.fixed-table-toolbar > .page-list').html($('.pagination-detail > .page-list')).find('.btn-group ').removeClass('dropup').addClass('dropdown');
            else
        ...
        onPostBody: function(data) {
            ...
            if ($('input[data-field=output_rows]').is(":checked"))
                $('.total_exported small').show();
            else
                $('.total_exported small').hide();
...
// FORMATTERS
function totalexportedFormatter(value, row, index) {
...
function actionsFormatter(value, row, index) {
    return [
        '<button class="settings btn btn-sm btn-default" title="Settings" data-tooltip="true">',
            '<i class="icon wb-settings"></i>',
        '</button>',
        '<button class="log btn btn-sm btn-default" title="Log" data-tooltip="true">',
            '<i class="icon wb-clipboard"></i>',
        '</button>',
        '<button class="remove btn btn-sm btn-default" title="Remove" data-tooltip="true">',
...
$('#toolbar .bulk-delete').on('click', function() {
    ...
    if ($('#exportsTable th[data-field="state"]').length) {

        var obj_selected = $('#exportsTable').bootstrapTable('getAllSelections');
        ...
        if (delete_ids.length) {
            ...
            $('#exportsTable').bootstrapTable('hideColumn', 'state');
            $('#toolbar .bulk-delete').removeClass('active');
        }

    } else {
        $('#exportsTable').bootstrapTable('showColumn', 'state');
...
window.actionsEvents = {
    ...
    'click .log': function (e, value, row, index) {
        window.location.href = 'export-log.php?id=' + row.id;
    },
...
$('#deleteModal').on('click', '.delete', function(e) {
    ...
    $('#exportsTable').bootstrapTable('remove', {field: 'id', values: delete_ids});

    $.ajax({
        type: "POST",
        url: "?action=export_list_delete",
        data: {
            'ids' : delete_ids
        }
    }).done(function( out ) {
